...
%>
	<h1>예약 페이지</h1>
	<p>이름:  <%= (c != null ? c.getName() : "이름 없음") %> </p>
	<form action="/prj3_1/bookingcheck.do" method="POST">
	<%if (c != null && c.getPasno() != null){%>
	    <p name="passportText" value="<%= c.getPasno() %>">여권번호: <%= c.getPasno() %></p>
    ...
    <% }%>
	<p>성별: <%=( c != null ? c.getGender() : "성별 입력 없음") %></p>
	<p>항공편번호: <%=fcode %></p>
	<p>출발지: <%= ( sc != null ? sc.getDep() : "출발지 없음") %></p>
	<p>도착지: <%= ( sc != null ? sc.getArr()  : "도착지 없음") %> </p>
	<p>출발일: <%= ( sc != null ? sc.getSday()  : "출발일 없음") %></p>
	<p>도착일: <%= ( sc != null ? sc.getEday()  : "도착일 없음") %></p>
	<p>출발시각: <%= ( sc != null ? sc.getStime() : "출발 시간 없음") %></p>
	<p>출발시각: <%= ( sc != null ? sc.getEtime() : "도착 시간 없음") %></p>
	<p>출발시각: <%= ( sc != null ? seatnum : "좌석 없음") %></p>
	<p>출발시각: <%= ( sc != null ? grade : "등급 없음") %></p>
	<div class="resres">
